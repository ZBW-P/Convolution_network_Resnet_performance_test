...
class resnet_18(nn.Module):
    def __init__(self, num_blocks,inpt_size,output_size,stride,kenerl_size,padding,class_label):
        ...
        self.dens=nn.Linear(output_channel,class_label)
...
def train(model: nn.Module, train_dataloader: DataLoader, optimizer,criterion, epochs=5, device="cuda"):
    ...
    for epoch in range(epochs):
        ...
        print(
            f":Average training time {total_time/(epoch+1):.8f}"
        )
        print(
            f"Epoch [{epoch+1}/{epochs}] - Loss: {epoch_loss:.4f}, Accuracy: {epoch_acc:.2f}%"
        )
        Average_running.append(total_time/(epoch+1))
        Epoch_loss.append(epoch_loss)
        Epoch_acc.append(epoch_acc)
        
        
    return Average_running,Epoch_loss,Epoch_acc
...
if __name__ == "__main__":
    ...
    if args.optimizer=='sgd':
        ...
        optim=torch.optim.Adam(model.parameters(),lr=0.1,betas=(0.9, 0.999),weight_decay=5e-4)
    ...
    Average_running,Epoch_loss,Epoch_acc=train(model, train_dl, optim, loss, epoch, device)
    epochs = range(1, epoch+1)

    plt.figure(figsize=(12,4))

    plt.subplot(1,3,1)
    plt.plot(epochs, Average_running, marker='o')
    plt.title("Epoch Running Time")
    plt.xlabel("Epoch")
    plt.ylabel("Time (s)")
    ...
    plt.tight_layout()
    plt.savefig(f"{args.optimizer} graph")
