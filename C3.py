...
def train(model: nn.Module, train_dataloader: DataLoader, optimizer,criterion, epochs=5, device="cuda"):
    ...
    return Epoch_time, Load_time,Train_time

def Train_with_profiler(model: nn.Module, train_dataloader: DataLoader, optimizer,criterion,worker, epochs=5, device="cuda"):
    with torch.profiler.profile(
        schedule=torch.profiler.schedule(wait=0, warmup=1, active=3, repeat=1),
        on_trace_ready=torch.profiler.tensorboard_trace_handler("Log3", worker_name=worker),
        record_shapes=True,
        with_stack=True
    ) as prof:
        for epoch in range(epochs):
...
if __name__ == "__main__":
    ...
    Total_time_save_workers=[]
    work_list=[]
    if args.profiler:
        while workers <= 20:
            print(f"workers: { workers }")
            train_dl, test_dl = train_loader(root, batch_size, workers)
            prof=Train_with_profiler(model, train_dl, optim, loss,workers, epoch, device)
            data_loading_time = [x for x in prof.key_averages() if x.key == "data_loading"][0].cpu_time_total / 1e6
            Total_time_save_workers.append(data_loading_time)
            work_list.append(workers)
            if len(Total_time_save_workers) > 1:
                if abs(Total_time_save_workers[-2]-Total_time_save_workers[-1])<0.00001:
                    break
            workers+=4
    else:
        while workers <= 20:
            print(f"workers: { workers }")
            train_dl, test_dl = train_loader(root, batch_size, workers)
            _,Load_time,_=train(model, train_dl, optim, loss, epoch, device)
            Total_time_save_workers.append(Load_time)
            work_list.append(workers)
            if len(Total_time_save_workers) > 1:
                if abs(Total_time_save_workers[-2]-Total_time_save_workers[-1])<0.00001:
                    break
            workers+=4
    plt.plot(work_list, Total_time_save_workers, marker='o')
    plt.xlabel("number of workers")
    plt.ylabel("I/O time ")
    plt.title("The I/O time vs Number of workers")
    plt.grid(True)
    plt.savefig("IO_image.png")
    lowest=float('inf')
    lowest_index=-1
    for index, value in enumerate(Total_time_save_workers):
        if lowest>value:
            lowest=value
            lowest_index=index
    print(f"The lowest I/O time is {lowest:.8f} and the number of worker is {work_list[lowest_index]}")
