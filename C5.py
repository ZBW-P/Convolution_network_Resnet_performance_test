...
def Train_with_profiler(model: nn.Module, train_dataloader: DataLoader, optimizer, criterion,epochs=5, device="cuda"):
    ...
    for epoch in range(epochs):
        ...
        with torch.profiler.profile(
            ...
            on_trace_ready=torch.profiler.tensorboard_trace_handler(f'./Log5'),
            record_shapes=True,
            with_stack=True
        ) as prof:
...
if __name__ == "__main__":
    ...
    if args.profiler:
        average_time=Train_with_profiler(model, train_dl, optim, loss, epoch, device)
    else:
        average_time=train(model, train_dl, optim, loss, epoch, device)
    
    print(f"The device is {device}: average running time over 5 epochs is : {average_time}")
