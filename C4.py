...
def Train_with_profiler(model: nn.Module, train_dataloader: DataLoader, optimizer, criterion,epochs=5, device="cuda"):
    for epoch in range(epochs):
        ...
        with torch.profiler.profile(
            ...
            on_trace_ready=torch.profiler.tensorboard_trace_handler(f'./Log4'),
            record_shapes=True,
            with_stack=True
        ) as prof:
            for batch_idx, batch in enumerate(train_dataloader):
        ...
        data_loading_time = sum([x.cpu_time_total for x in prof.key_averages() if x.key == "data_loading"]) / 1e6
        training_time = sum([x.cpu_time_total for x in prof.key_averages() if x.key == "training_step"]) / 1e6
        epoch_loss = train_loss / len(train_dataloader)
        epoch_acc = 100.0 * correct / total

        print(f"Epoch [{epoch+1}/{epochs}] - Loss: {epoch_loss:.4f}, Accuracy: {epoch_acc:.2f}%")
        print(f"Epoch_time: {epoch_time:.8f}")
        print(f"Load_time: {data_loading_time:.8f}")
        print(f"Train_time: {training_time:.8f}")
